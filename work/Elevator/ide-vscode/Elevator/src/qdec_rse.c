...
#define DT_DRV_COMPAT hevs_rse_qdec
...
static int qdec_stm32_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	dev_data->direction = LL_TIM_GetDirection(dev_cfg->timer_inst);
			...
			struct sensor_value *val)
{
	...
	if (chan == SENSOR_CHAN_ROTATION) {
		...
		val->val1 = (int32_t)dev_data->direction;
		val->val2 = 0;
	} else {
		return -ENOTSUP;
	}
...
DT_INST_FOREACH_STATUS_OKAY(QDEC_RSE_INIT)
